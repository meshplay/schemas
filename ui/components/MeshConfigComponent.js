...
function MeshplaySettingsNew({
  ...
  useEffect(() => {
    setTableData();
    // k8sconfig.forEach((ctx) => {
    //   const tempSubscription = fetchMeshplayOperatorStatus({ k8scontextID: ctx.id }).subscribe({
    //     next: (res) => {
  ...
  const handleOperatorClick = (index) => {
    ...
    const tempSubscription = fetchMeshplayOperatorStatus({ k8scontextID: ctxId }).subscribe({
      next: (res) => {
    ...
    NatsStatusQuery({ connectionID: contexts[index].connection_id }).subscribe({
      next: (res) => {
        ...
        if (
          ...
          handleError('Meshplay Broker could not be reached')(
    ...
    MeshsyncStatusQuery({ connectionID: connectionID }).subscribe({
      next: (res) => {
        ...
        if (res.controller.name === 'MeshSync' && res.controller.status.includes('Connected')) {
          ...
          notify({
            message: `MeshSync is not publishing to Meshplay Broker`,
  ...
  return (
    <div style={{ display: 'table', tableLayout: 'fixed', width: '100%' }}>
      <DataTable
        title={<MeshplaySettingsEnvButtons />}
        columns={columns}
        data={data}
        options={options}
