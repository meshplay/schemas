...
Examples:
	...
	1. ./main https://docs.google.com/spreadsheets/d/e/2PACX-1vSgOXuiqbhUgtC9oNbJlz9PYpOEaFVoGNUFMIk4NZciFfQv1ewZg8ahdrWHKI79GkKK9TbmnZx8CqIe/pub\?gid\=0\&single\=true\&output\=csv --system docs khulnasoft/src/collections/integrations khulnasoft.com/integrations docs/ --published-only
...
func docsUpdater(output []map[string]string) {
	...
	for _, out := range output {
		...
		// Write SVGs to Meshplay website
		err = os.MkdirAll(filepath.Join(pathToIntegrationsMeshplay, "../", "images"), 0777)
		if err != nil {
			panic(err)
		}

		err = pkg.WriteSVG(filepath.Join(pathToIntegrationsMeshplay, "../", "images", modelName+"-color.svg"), svgcolor) //CHANGE PATH
		if err != nil {
			panic(err)
		}

		err = pkg.WriteSVG(filepath.Join(pathToIntegrationsMeshplay, "../", "images", modelName+"-white.svg"), svgwhite) //CHANGE PATH
		if err != nil {
			panic(err)
		}

		// Write SVGs to Meshplay docs
		err = os.MkdirAll(filepath.Join(pathToIntegrationsMeshplayDocs), 0777)
		if err != nil {
			panic(err)
		}

		err = pkg.WriteSVG(filepath.Join(pathToIntegrationsMeshplayDocs, modelName+"-color.svg"), svgcolor) //CHANGE PATH
		if err != nil {
			panic(err)
		}

		err = pkg.WriteSVG(filepath.Join(pathToIntegrationsMeshplayDocs, modelName+"-white.svg"), svgwhite) //CHANGE PATH
		if err != nil {
			panic(err)
		}
	}
