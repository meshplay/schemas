...
<!-- GUIDES -->
  <div class="section">
    ...
    <ul>
      {% for item in sorted_pages %}
      {% if item.type=="guides" and item.category!="meshplayctl" and item.category!="infrastructure" and item.category!="troubleshooting" and item.category!="performance" and item.category!="configuration" and item.language=="en" -%}
        <li><a href="{{ site.baseurl }}{{ item.url }}">{{ item.title }}</a>
        </li>
        {% endif %}
      {% endfor %}
...
<img src="https://khulnasoft.com/assets/images/khulnasoft/meshplay-logo-shadow-light-white-text-side.svg" width="60%" />
<h1>Documentation</h1>
</div> -->
