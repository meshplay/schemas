...
<table class="table table-striped" >
  ...
  <th><img style="height: 1.5rem; vertical-align: text-bottom;" src="{{site.baseurl}}/assets/img/service-meshes/osm.svg" /><a href="{{ site.repo }}-osm">meshplay-osm</a></th>
  ...
  <tr class = "first-row">
    ...
    {%if k8s.name != "v1.21.0"%}
      ...
      {% for k8s_item in k8s_items %}
        ...
        {% elsif k8s_item.name == "meshplay-osm" %}
          ...
          {%else%}
            ...
            {% assign osm_items = k8s_item.items | sort: "meshplay-component-version" | reverse  %}
          {%endif%}
      ...
      {%if osm_size and osm_size !=0%}
        ...
        <td style="text-align:center" onclick = "clickIcon(`meshplay-osm`)" class = "compatibility">{{osm_percentage}}%</td>
      {%else%}
