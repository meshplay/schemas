...
func (r *Resolver) getOperatorStatus(ctx context.Context, _ models.Provider, ctxID string) (*model.MeshplayControllersStatusListItem, error) {
	...
	controllerHandlersPerContext := handler.MeshplayCtrlsHelper.GetControllerHandlersForEachContext()
...
func (r *Resolver) getMeshsyncStatus(ctx context.Context, provider models.Provider, connectionID string) (*model.OperatorControllerStatus, error) {
	...
	if connectionID != "" {
		...
		if err != nil {
			return nil, model.ErrMeshplayClientNil
		}
		kubeclient, err = k8scontext.GenerateKubeHandler()
		if err != nil {
			return nil, model.ErrMeshplayClient(err)
		}
	}
