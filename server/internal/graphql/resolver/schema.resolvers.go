...
func (r *queryResolver) GetOperatorStatus(ctx context.Context, k8scontextID string) (*model.MeshplayControllersStatusListItem, error) {
	provider := ctx.Value(models.ProviderCtxKey).(models.Provider)
...
func (r *subscriptionResolver) SubscribeMeshplayControllersStatus(ctx context.Context, k8scontextIDs []string) (<-chan []*model.MeshplayControllersStatusListItem, error) {
	...
	statusMapPerCtx := make(map[string]map[models.MeshplayController]controllers.MeshplayControllerStatus)
	// initialize the map
	for ctxID, ctrlHandlers := range controllerHandlersPerContext {
		for controller, handler := range ctrlHandlers {
			if _, ok := statusMapPerCtx[ctxID]; !ok {
				statusMapPerCtx[ctxID] = make(map[models.MeshplayController]controllers.MeshplayControllerStatus)
			}
...
func (r *subscriptionResolver) SubscribeMeshSyncEvents(ctx context.Context, k8scontextIDs []string, eventTypes []model.MeshSyncEventType) (<-chan *model.MeshSyncEvent, error) {
	...
	meshSyncDataHandlers := handler.MeshplayCtrlsHelper.GetMeshSyncDataHandlersForEachContext()
...
func (r *subscriptionResolver) SubscribePerfResult(ctx context.Context, id string) (<-chan *model.MeshplayResult, error) {
	panic(fmt.Errorf("not implemented"))
}
