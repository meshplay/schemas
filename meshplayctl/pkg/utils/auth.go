...
// NewRequest creates *http.Request and handles adding authentication for Meshplay itself
...
func GetTokenLocation(token config.Token) (string, error) {
	...
	return filepath.Join(MeshplayFolder, location), nil
}
...
func UpdateAuthDetails(filepath string) error {
	...
	req, err := http.NewRequest("GET", mctlCfg.GetBaseMeshplayURL()+"/api/user/token", bytes.NewBuffer([]byte("")))
	if err != nil {
...
func GetProviderInfo(mctCfg *config.MeshplayCtlConfig) (map[string]Provider, error) {
	...
	resp, err := http.Get(mctCfg.GetBaseMeshplayURL() + "/api/providers")
	if err != nil {
		return nil, err
	}
...
func getTokenObjFromMeshplayServer(mctl *config.MeshplayCtlConfig, provider, token string) ([]byte, error) {
	req, err := http.NewRequest(http.MethodGet, mctl.GetBaseMeshplayURL()+"/api/token", nil)
	if err != nil {
		return nil, err
	}
