...
meshplayctl app offboard -f [filepath]
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if cmd.Flags().Changed("file") && file == "" {
			const errMsg = `Usage: meshplayctl app offboard -f [filepath]`
		...
		isID := false
		if len(args) > 0 {
			app, isID, err = utils.ValidId(mctlCfg.GetBaseMeshplayURL(), args[0], "pattern")
			if err != nil {
				utils.Log.Error(err)
				return nil
			}
		...
		if isID {
			err := utils.DeleteConfiguration(mctlCfg.GetBaseMeshplayURL(), app, "pattern")
			if err != nil {
				utils.Log.Error(err)
