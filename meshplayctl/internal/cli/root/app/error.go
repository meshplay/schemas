...
const (
	errAppMsg = `Usage: meshplayctl app import -f [file/url] -s [source-type]
Example: meshplayctl app import -f ./application.yml -s "Kubernetes Manifest"`

	errOnboardMsg = `Usage: meshplayctl app onboard -f [filepath] -s [source type]
Example: meshplayctl app onboard -f ./application.yml -s "Kubernetes Manifest"
...
func ErrAppFound() error {
	return errors.New(ErrAppFoundCode, errors.Alert,
		...
		[]string{"Provide an app name. Use `meshplayctl app list`to display list of applications"},
	)
}
...
func ErrInvalidAppNameOrID(err error) error {
	return errors.New(ErrInvalidAppNameOrIDCode, errors.Alert,
		...
		[]string{"Run `meshplayctl app view --all` to view all applications"})
}
