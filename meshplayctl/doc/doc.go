...
func doc() {
	...
	cmd := root.TreePath() // Takes entire tree of meshplayctl commands
...
func GenMarkdownCustom(cmd *cobra.Command, w io.Writer, manuallyAddedContent map[int]string) error {
	...
	if hasSeeAlso(cmd) {
		...
		buf.WriteString("Go back to [command reference index](/reference/meshplayctl/), if you want to add content manually to the CLI documentation, please refer to the [instruction](/project/contributing/contributing-cli#preserving-manually-added-documentation) for guidance.")
		buf.WriteString("\n")
	}
